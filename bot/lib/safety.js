...
class Safety {
  ...
  async checkVolatility(pair, currentPrice, thresholdPercent = 2) {
    ...
    if (Math.abs(changePercent) >= thresholdPercent) {
      ...
      await notify.sendDiscord(
        ...
        `Bot を一時停止しました。\n\n` +
        `<@1464669395543654430> ステラ、状況確認して再開判断お願い！`
      );
  ...
  async recordTrade(profit, maxDailyLoss = 5000) {
    ...
    if (this.state.dailyLoss >= maxDailyLoss) {
      ...
      await notify.sendDiscord(
        ...
        `<@1464669395543654430> ステラ、原因分析お願い！`
      );
  ...
  // 自動再開チェック（停止中に呼ばれる）
  async checkAutoResume(pairs, thresholdPercent = 1.0) {
    if (!this.state.isPaused || this.state.pauseReason !== 'volatility') {
      return { resumed: false };
    }

    // 停止から最低10分は待つ
    const pausedAt = new Date(this.state.pausedAt).getTime();
    const minWaitMs = 10 * 60 * 1000;
    if (Date.now() - pausedAt < minWaitMs) {
      return { resumed: false, reason: 'waiting' };
    }

    // 全ペアのボラティリティをチェック
    for (const pair of pairs) {
      const history = this.state.priceHistory[pair] || [];
      ...
      if (rangePercent > thresholdPercent) {
        return { resumed: false, reason: 'still_volatile', pair, volatility: rangePercent };
      }
    }
